...
int main(int argc, char* argv[]) {
    ...
    while(not_end){
        semaphore_up();
        if(mem_access.memaddr[2]>0){
            int index = mem_access.memaddr[1];
            int number = buffer[index];

            mem_access.memaddr[1] = (mem_access.memaddr[1] + 1) % 50;
            mem_access.memaddr[2]--;

            struct timeval time_val;
            gettimeofday(&time_val, NULL);
            if(number % 2 == 0)
                printf("(%d %ld.%ld) liczba %d na pozycji %d - parzysta. Pozostało zadań: %d\n",
                    getpid(), time_val.tv_sec, time_val.tv_usec, number, index, mem_access.memaddr[2]);
            else
                printf("(%d %ld.%ld) liczba %d na pozycji %d - nieparzysta. Pozostało zadań: %d\n",
                       getpid(), time_val.tv_sec, time_val.tv_usec, number, index, mem_access.memaddr[2]);
