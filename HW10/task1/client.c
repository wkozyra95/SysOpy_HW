...
void* run_sender(void* data){
    ...
    const struct sockaddr* server = get_server_address();
    socklen_t addr_size = get_server_address_size();
    while(true){
        fgets(new_request.message_body, MAX_MESSAGE, stdin);
        check(
                sendto(C.socket_fd, &new_request, sizeof(request_t), 0, server, addr_size) <= 0,
                "Unable to send"
        );
    }
}
...
void* run_receiver(void* data){
    ...
    struct sockaddr* server = get_server_address();
    socklen_t addr_size = get_server_address_size();
    while(true){
        fd_set fd_listener;
        FD_ZERO(&fd_listener);
        ...
        addr_size = get_server_address_size();
        if(FD_ISSET(C.socket_fd, &fd_listener)){
            check(
                    recvfrom(C.socket_fd, &received_broadcast, sizeof(request_t), MSG_DONTWAIT, server, &addr_size) <= 0,
                    "Unable to receive message"
            );
            printf("Received broadcast\n user: %s\n message:%s",
                   received_broadcast.username, received_broadcast.message_body);
            check(addr_size > get_server_address_size(), "Wrong address size");
        }
        addr_size = get_server_address_size();
